...
1. **Sender Setup:**
    - Open <a href="index.html" target="_blank">index.html</a> in the sender's browser.
    - Upload the file that needs to be transferred.

2. **Receiver Setup:**
    - Open <a href="index.html?mode=receiver" target="_blank">index.html?mode=receiver</a> in the receiver's browser (or open `index.html` and switch to Receiver).
...
Use the Sender / Receiver buttons to switch modes.

All runtime dependencies live in the `vendor/` folder, so the app does not require network access after cloning.
...
1. Open `index.html` and select **Receiver** (or open `index.html?mode=receiver`).
